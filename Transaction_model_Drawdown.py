...
def drawdown():
    ...
    for i in range(1, 3791):
        if(i % 48 == 0):
            ...
            money_History.append(todayMoney)
    ...
    drawdown = 0.0
    maxDrawdown = 0.0
    for i in range(0, len(money_History)-1):
        for j in range(i, len(money_History)):
            drawdown = (money_History[i]-money_History[j]) / money_History[i]
            if(drawdown > maxDrawdown):
                maxDrawdown = drawdown

    print("最大回撤为：" + maxDrawdown.__str__())
    return maxDrawdown
